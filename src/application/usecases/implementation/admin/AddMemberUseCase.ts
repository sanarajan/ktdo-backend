...
import { IAddMemberUseCase } from '../../interface/admin/IAddMemberUseCase';
...
export class AddMemberUseCase implements IAddMemberUseCase {
  ...
  async execute(data: Partial<Driver>, file?: Express.Multer.File): Promise<Driver> {
    ...
    try {
      await this.emailService.sendAdminMemberApprovedEmail(data.email!, data.name!);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      this.logger.warn('Failed to send admin member approved email', message);
      // Don't throw - email failure shouldn't prevent member creation
    }
